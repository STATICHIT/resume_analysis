    ...
    <div>
      ...
      <div v-for="item in state.project_experience" :key="item">
        ...
        <ul class="project">
          ...
          <li><span>项目描述: </span>{{ item.project_msg }}
          </li>
          <li><span>项目职责:</span>
            {{ item.project_work }}
          </li>
...
const state = reactive({
  project_experience:[{
    ...
    work_msg:'Web前端/网页前端',
    work:'Web前端开发',
    project_msg:'该项目是第十四届“中国大学生服务外包创新创业大赛”的参赛作品 ， 主要使用的技术栈是Vue3+TypeScript+Pinia+axios，其主要功能是提供海量、安全以及高可用的对象存储服务，除此之外还提供了对象级别的权限管理，对整个平台数据的数据监控和日志文件的实时查询，以及对对象数据进行二次处理的功能。',
    project_work:'参与需求评审，完成项目原型、接口文档设计，负责项目整体前端页面需求的开发，进行前后端联调测试和部署等。',
    project_lights:'基于RBAC权限模型，实现页面、菜单和按钮级别的权限管理基于CanvasAPI实现对于图片文件的压缩、编辑、水印和格式转换等功能。针对大文件传输场景，采用文件切片、秒传、断点续传来优化该业务场景搭建监控SDK，对项目中的异常、用户行为以及PV/UV数据进行上报，使用可视化技术ECharts.js展示报表数据'
  }],
  json:{
    "project_experience": [],
    "contact_info": {
